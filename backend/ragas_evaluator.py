...
def _load_langchain_docs() -> List[Any]:
    ...
    data_file = "./data/mock_data_large.json"
...
def combine_short_docs(docs, min_tokens=100):
    combined_docs = []
    current_combined = ""
    for doc in docs:
        current_combined += " " + doc.page_content.strip()
        if len(current_combined.split()) >= min_tokens:
            # If the combined document has enough tokens, add it to the list
            combined_docs.append(Document(page_content=current_combined.strip(), metadata=doc.metadata))
            current_combined = ""  # Reset for the next combination

    # If there's any remaining short text, you can decide to skip or add it
    if len(current_combined.split()) >= min_tokens:
        combined_docs.append(Document(page_content=current_combined.strip(), metadata={}))
...
def build_dataset(
    ...
    if use_ragas_generator:
        ...
        for example in ragas_testset:
            print(vars(example))
            query = example.eval_sample.user_input
